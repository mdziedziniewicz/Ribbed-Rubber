...
def ReadJoints(dir,file):
    ...
    #CHECK THERE IS EXISTENT JOINT LIBRARY FILR. IF THERE ISN'T RETURN WRITE ERROR FILE    
...
def PurgePoly(reference):
    
    npoly = []
    
    npoly.append(reference[0])
    
    for i in range(len(reference)-1):
        
        if rs.Distance(reference[i],reference[i+1])>0:
            
            npoly.append(reference[i+1])
...
def Arm(refpt,dir,arrlist):
    ...
    for j in range(len(res[1])):
        upl.append(rs.VectorAdd(res[1][j],[0,0,-1*trim]))
        left.append(res[1][j])
        bupl.append(rs.VectorAdd(res[1][j],[0,0,-1*trim]))
        bleft.append(res[1][j])
    for j in range(len(res[2])):
        upr.append(rs.VectorAdd(res[2][j],[0,0,-1*trim]))
        right.append(res[2][j])
        bupr.append(rs.VectorAdd(res[2][j],[0,0,-1*trim]))
        bright.append(res[2][j])
    
    
    store = [[left,right],[upl,upr]]
    firstGo = [[bleft,bright],[bupl,bupr]]
